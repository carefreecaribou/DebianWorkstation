...
touch ~/.config/environment.d/90-login.conf
echo 'gnome-terminal -- bash -c "cd ~/Development/Debain12PentestWorkstation && ansible-playbook main.yml --ask-become-pass; bash"' >> ~/.config/environment.d/90-login.conf
sudo reboot now